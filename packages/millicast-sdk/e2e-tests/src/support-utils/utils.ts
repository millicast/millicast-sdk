...
export async function getDiagnose(scenarioWorld: ScenarioWorld) {
...
export async function getMediaStream(scenarioWorld: ScenarioWorld) {
  const page = scenarioWorld.page;
  const mediaStream = {
    active: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.active',
    ),
    id: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.id',
    ),
    mediaTracks: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.getTracks()',
    ),
  };
...
export async function getVideoTrack(scenarioWorld: ScenarioWorld) {
  const page = scenarioWorld.page;
  const videoTrack = {
    kind: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.getVideoTracks()[0].kind',
    ),
    readyState: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.getVideoTracks()[0].readyState',
    ),
  };
  return videoTrack;
}

export async function getAudioTrack(scenarioWorld: ScenarioWorld) {
  const page = scenarioWorld.page;
  const audioTrack = {
    kind: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.getAudioTracks()[0].kind',
    ),
    readyState: await page.evaluate(
      'document.getElementsByTagName("video")[0].srcObject.getAudioTracks()[0].readyState',
    ),
  };
  return audioTrack;
}
