...
describe('Extract user unregistered data in SEI from H26x frame', () => {
  ...
  xit('should extract user uregistered data from H264/AVC frame', () => {
  ...
  xit('should extract user unregistered data from H265/HEVC frame', () => {
    const frameBuffer = Buffer.concat([
      Buffer.from([0, 0, 0, 1]),
  ...
  xit('should extract user unregistered data when there is emulation_prevention_three_byte', () => {
...
describe('Extract pic_timing SEI from h26x sample', () => {
  xit('should extract expected pic_timing SEI from h264 sample', () => {
    const output = []
    for (let i = 1; i <= 4; i++) {
