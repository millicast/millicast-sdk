...
const feature = loadFeature('../features/SdpStereo.feature', { loadRelativePath: true, errors: true })

defineFeature(feature, test => {
  test('Set stereo', ({ given, when, then }) => {
    let localSdp

    given('a local sdp without stereo', async () => {
      ...
      expect(localSdp).toEqual(expect.not.stringMatching('useinbandfec=1; stereo=1'))
    })

    when('I want to set stereo', async () => {
      localSdp = SdpParser.setStereo(localSdp)
    })

    then('returns the sdp with stereo support', async () => {
      expect(localSdp).toEqual(expect.stringMatching('useinbandfec=1; stereo=1'))
    })
  })
})
