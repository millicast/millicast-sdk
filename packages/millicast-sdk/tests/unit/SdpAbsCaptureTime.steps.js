...
defineFeature(feature, (test) => {
  test('Set abs-capture-time', ({ given, when, then }) => {
    ...
    given('a local sdp without the header extension', async () => {
      ...
      expect(localSdp).toEqual(
        expect.not.stringMatching('http://www.webrtc.org/experiments/rtp-hdrext/abs-capture-time')
      )
    })
    ...
    then('returns the sdp with the header extension', async () => {
      expect(localSdp).toEqual(
        expect.stringMatching('http://www.webrtc.org/experiments/rtp-hdrext/abs-capture-time')
      )
    })
  })
})
