class MillicastPublishUserMediaTest {
  ...
  async testStart(options = undefined) {
    ...
    try{
      const response = await this.millicastPublishUserMedia.broadcast(broadcastOptions)
  ...
  testMuteAudio(checkboxObject){
    const muted = this.millicastPublishUserMedia.muteMedia('audio', checkboxObject.checked)
    console.log('MuteMedia audio response:', muted)
  }

  testMuteVideo(checkboxObject){
    const muted = this.millicastPublishUserMedia.muteMedia('video', checkboxObject.checked)
    console.log('MuteMedia video response:', muted)
  }

  async testChangeAudio(selectObject) {
    const deviceId = selectObject.value
    this.mediaStream = await this.millicastPublishUserMedia.updateMediaStream('audio', deviceId)
  ...
  async testChangeVideo(selectObject) {
    const deviceId = selectObject.value
    this.mediaStream = await this.millicastPublishUserMedia.updateMediaStream('video', deviceId)
