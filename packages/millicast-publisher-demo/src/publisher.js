    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: [],
          backgroundColor: 'rgba(0, 50, 132, 0.2)',
...
async function onRefresh(observer) {
  try {
    const stats = await millicastPublish.webRTCPeer.peer.getTransceivers()[1].sender.getStats();
    observer.disconnect();
    observer.observe({ entryTypes: ['RTCVideoSender', 'media'] });
    processStats(stats);
  } catch (err) {
